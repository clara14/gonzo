...
template <class T>
string AddressAsString(DListNode<T> * in)
{
ostringstream temp;
temp << ((void * )in);
return temp.str();
...
template <class T>
string DrawNode (DListNode<T> *in, int x, int y )
  {
  ostringstream ss;
...
template <class T>
void ToHTMLFile(DLinkedList<T> & list,char const * filename)
  {
  char filename2[300];
  ...
  js += DrawText("Tail:" + AddressAsString(list.tail) ,300,40,"#00cc00");
