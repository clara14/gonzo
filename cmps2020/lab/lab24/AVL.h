...
class AVL
  ...
  private:
    ...
    TreeNode<T>* insertSubtree(TreeNode<T> *, T);
    ...
    void printSubtreeReverse(TreeNode<T> *);
  ...
  public:
    ...
    template <class T2>
    friend void ToHTMLFile( AVL<T2> &,char const *);
  };


template <class T>
TreeNode<T> *rightRotate(TreeNode<T>* in)
{
    TreeNode<T>* one = in->left;
    TreeNode<T>* two = one->right;
    one->right = in;
    in->left = two;
    return one;
}
...
template <class T>
TreeNode<T> *leftRotate(TreeNode<T>* in)
{
    TreeNode<T>* one = in->right;
    TreeNode<T>* two = one->left;
    one->left = in;
    in->right = two;
    return one;
}
...
template <class T>
void AVL<T>::deallocateSubtree(TreeNode<T> *subtree)
  {
...
template <class T>
TreeNode<T>* AVL<T>::createNode(T elem)
  {
    LogStart();
...
template <class T>
bool AVL<T>::insert(T elem)
  {

    LogStart(elem);
    
    root = insertSubtree(root,elem);
    LogEndReturning(true);
    return true;
  }

template <class T>
TreeNode<T>* AVL<T>::insertSubtree(TreeNode<T> *subtree, T elem)
  {
    LogStart(subtree,elem);
    
    if(subtree==nullptr)
    {
      subtree = createNode(elem);
      LogEndReturning(subtree);
      return subtree;
    }
    if(subtree->data == elem)
    {
        cout << "Element " << elem << " already in tree.\n";
        LogEndReturning(subtree);
        return(subtree);
    }
    ...
    int l_height = height(subtree->left);
    int r_height = height(subtree->right);

    if(l_height - r_height > 1 && elem < subtree->left->data)
        subtree = rightRotate(subtree);

    if(r_height - l_height > 1 && elem > subtree->right->data)
...
template <class T>
TreeNode<T>* AVL<T>::search(T elem)
  {
  // if AVL is empty throw "Binary Search Tree Empty, search not allowed"
// otherwise call searchSubtree
    LogStart(elem);
    if(empty())
    {
        throw "Binary Search Tree Empty, search not allowed";
    }
    TreeNode<T>* temp=nullptr;
    temp = searchSubtree(root,elem);
    LogEndReturning(temp);
    return temp;
  }

template <class T>
TreeNode<T>* AVL<T>::searchSubtree(TreeNode<T> *subtree, T elem)
  {
...
// search the left side .. other wise search the right side
    ...
    if(elem > subtree->data && subtree->right!=nullptr)
    {
        temp = searchSubtree(subtree->right,elem);
    }
    if(elem < subtree->data && subtree->left!=nullptr)
    {
        temp = searchSubtree(subtree->left,elem);
    }
    LogEndReturning(temp);
    return temp;
  }
...
template <class T>
void AVL<T>::printSubtree(TreeNode<T> *subtree)
  {
...
// if the right node pointer is not nullptr call printSubtree on right
    ...
    cout << "Node: " << subtree << " Data: " << subtree->data << " Left: "
         << subtree->left << " Right: " << subtree->right << endl;
    if(subtree->right!=nullptr)
...
template <class T>
void AVL<T>::printSubtreeReverse(TreeNode<T> *subtree)
{
    LogStart(subtree);
    if(subtree->right!=nullptr)
        printSubtreeReverse(subtree->right);
    cout << "Node: " << subtree << " Data: " << subtree->data << " Left: "
         << subtree->left << " Right: " << subtree->right << endl;
    if(subtree->left!=nullptr)
        printSubtreeReverse(subtree->left);
    LogEnd();
}

template <class T>
bool AVL<T>::remove(T elem)
  {
...
template <class T>
bool AVL<T>::remove(TreeNode<T>* ptr, T elem)
  {
  LogStart(elem);
...
template <class T>
TreeNode<T> * AVL<T>::iterativeSearch(T input)
  {
  // if the AVL is empty    throw "Binary Search Tree empty, search not allowed";
...
// are searching for
    ...
    while(temp->data!=input && temp!=nullptr)
    {
        if(input>temp->data)
            temp = temp->right;
